...
using Fen_Test.Tests.TestDatas;
...
namespace Fen_Test.Tests
{
    ...
    /// Provides methods to easily work with <see cref="Fen_TestDbContext"/>.
    /// </summary>
    public abstract class AppTestBase : AbpIntegratedTestBase<Fen_TestTestModule>
    {
        ...
        {
            void NormalizeDbContext(Fen_TestDbContext context)
            {
        ...
        protected void UsingDbContext(Action<Fen_TestDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<Fen_TestDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<Fen_TestDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<Fen_TestDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<Fen_TestDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<Fen_TestDbContext>())
                {
                    action(context);
                    context.SaveChanges();
                }
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<Fen_TestDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<Fen_TestDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<Fen_TestDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<Fen_TestDbContext, Task<T>> func)
        {
            T result;
